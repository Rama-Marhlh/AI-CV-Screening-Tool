...
# Custom CSS for Streamlit layout
st.markdown("""
    <style>
    ...
    .metric-card {
        ...
        margin: 0.5rem 0;
        border: 1px solid #d1e7fd; /* Soft blue border for definition */
    ...
    .metric-card h3 {
        margin: 0;
        color: #ff6666; /* Bright red for headings */
        font-size: 1.2em;
    }

    .metric-card h2 {
        margin: 0;
        color: #ff6666; /* Bright red for values */
        font-size: 2em;
        font-weight: bold;
    }

    /* Specific styles for the metric value */
    .metric-card .value {
        font-size: 2.5em; /* Larger size for values */
        font-weight: bold;
        color: #00509e; /* Darker blue for emphasis */
    }

    /* Add color variations for different types of metrics */
    .metric-card.success {
    ...
    .metric-card.warning {
        border-left: 5px solid #ffc107; /* Yellow left border for warnings */
        background-color: #fff8e1; /* Light yellow background */
    }

    .metric-card.error {
        border-left: 5px solid #dc3545; /* Red left border for errors */
        ...
        if not st.session_state.resumes_data.empty and uploaded_job_desc:
            with st.spinner("Processing resumes... This might take a while."):
                ...
                for idx, row in st.session_state.sorted_resumes.iterrows():
                    # Extract the similarity score and language
                    similarity_score = row['similarity_score']
                    language = 'Arabic' if row['detected_language'] == 'ar' else 'English'

                    # Display the resume header and similarity score in a compact way
                    st.markdown(f"""
                         <div style="display: flex; justify-content: space-between; align-items: center; font-size: 0.9em; margin-bottom: 1em;">
                             <span><strong>Resume {idx} - {language}</strong></span>
                             <div style="background-color: #f0f2f6; padding: 0.3em 0.8em; border-radius: 5px; border: 1px solid #cce7ff; text-align: center; font-size: 0.9em;">
                                 <span style="font-size: 0.8em; color: #333;">Match Score</span><br>
                                 <span style="color: #0073e6; font-size: 1.5em; font-weight: bold;">{similarity_score:.2%}</span>
                             </div>
                         </div>
                         """, unsafe_allow_html=True)

                    # Display the resume text inside an expander (so the text remains hidden by default)
                    with st.expander(f"Resume {idx} Text"):
