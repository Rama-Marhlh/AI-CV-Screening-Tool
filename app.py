...
def detect_language(text: str) -> str:
    """
    Detects if text is primarily Arabic or English.
    """
...
if st.session_state.current_view == "upload":
        ...
        if not st.session_state.resumes_data.empty and uploaded_job_desc:
            with st.spinner("Processing resumes... This might take a while."):
                ...
                for idx, resume in enumerate(resumes):
                    ...
                    st.info(f"Resume {idx + 1} detected language: {'Arabic' if resume_lang == 'ar' else 'English'}")
    ...
    for idx, row in st.session_state.sorted_resumes.iterrows():
        with st.expander(f"Rank {idx } - Match Score: {row['similarity_score']:.2%} - Language: {'Arabic' if row['detected_language'] == 'ar' else 'English'}"):
